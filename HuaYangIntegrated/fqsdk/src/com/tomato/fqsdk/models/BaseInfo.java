...
import com.tomato.fqsdk.utils.CommonUtil;
...
public class BaseInfo {
	...
	public BaseInfo(Context context){
		...
		this.imei = imei;
		this.deviceid=CryptHelper.GetMD5Code(tm.getDeviceId()+Tools.getMacAddress(context));//
		...
		this.osVersion = android.os.Build.VERSION.RELEASE;
		 this.ip=Tools.GetIp(context);
		 try {
			 this.mac = Tools.getMacAddress(context);
		} catch (Exception e) {
			this.mac="-";
		}
		 try {
...
//			this.udid="0";
//		}
		this.udid=CommonUtil.getDeviceID(context);
